...
impl RRSimulation {
    ...
    pub fn simulate(&mut self, is_scheduling: bool) -> RRSimulationResult {
        ...
        let mut cores_release_time = BinaryHeap::<Reverse<FloatWrapper>>::new();
        for _ in 0..self.compute.borrow().cores_total() {
            cores_release_time.push(Reverse(FloatWrapper(0.)));
        }
        ...
        for result in &results_to_schedule {
            ...
            for _ in 0..result.spec.cores {
                start_time = start_time.max(cores_release_time.pop().unwrap().0 .0);
            }
            let finish_time = FloatWrapper(start_time + est_runtime);
            for _ in 0..result.spec.cores {
                cores_release_time.push(Reverse(finish_time.clone()));
            }
        }

        let mut shortfall = 0.;
        let mut busy_time = cores_release_time.peek().unwrap().0 .0;
        ...
        for _ in 0..self.compute.borrow().cores_total() {
            let core_released = cores_release_time.pop().unwrap().0 .0;
